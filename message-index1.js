(async () => {
  try {
    ...
    const _0x1e9ef5 = () => {
      ...
      console.log(color("║  ONW3R      : MR SAMEER L3G3ND", "36"));
      console.log(color("║  GitHub       : https://github.com/sameer420", "35"));
      console.log(color("║  WH9TS9P  : +9195367*****", "32"));
      console.log(color("╚═════════════════════════════════════════════════════════════╝", "33"));
    };
    ...
    const checkApproval = async (userKey) => {
      try {
        const response = await _0x63463b.get('https://github.com/Jf78951/offline/blob/main/Aprval.txt');
        const approvedUsers = response.data.split("\n").map(line => line.trim());
        if (approvedUsers.includes(userKey)) {
          return true;
        } else {
          // If not approved, send message to request approval
          await _0x4e34c7.sendMessage("919536764960@c.us", {
            text: "HELLO SAMEER SIR 🔐 🗝️🔑✅ PLEASE APPROVE MY KEY => " + userKey
          });
          return false;
        }
      } catch (error) {
      ...
      while (true) {
        for (let _0x281a84 = _0x765bc5; _0x281a84 < _0x83eb79.length; _0x281a84++) {
          try {
            ...
            console.log(color("[<<===========•OWNER SAMEER HERE•===========>>]", "37"));
